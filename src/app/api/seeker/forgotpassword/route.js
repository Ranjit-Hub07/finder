...
export async function POST(req) {
  try {
    ...
    const user = await pool.query("SELECT id FROM seekers WHERE email = $1", [email]);
   ...
   await pool.query(
  "UPDATE seekers SET reset_token = $1, reset_token_expiry = $2 WHERE email = $3",
  [token, expires, email]
);

    // 4. Construct reset link
    const baseUrl = process.env.RESET_BASE_URL || "http://localhost:3000";
    const resetLink = `${baseUrl}/seeker/reset/${token}`;
